###=================INITIALIZE ENVIORMENT===================
# server_env  <=== provided by user,for HOST:PORT-ENV
SERVICE=ads-admin
HOST=$(echo "${server_env}"|cut -d ':' -f1)
PORT=$(echo "${server_env}"|cut -d ':' -f2|cut -d '-' -f1)
ENV=$(echo "${server_env}"|cut -d ':' -f2|cut -d '-' -f2)
ID_FILE=~/.ssh/id_rsa_xhd2015
WAR_FILE=vivo-${SERVICE}/target/${SERVICE}.war
REMOTE_WAR_DIR=/root/ads
REMOTE_WAR_FILE=${SERVICE}.war
REMOTE_TOMCAT_DIR=/root/apache-tomcat-7.0.72

# empty
echo > env.sh

cat >> env.sh << EOF
ID_FILE="${ID_FILE}"
SERVICE="${SERVICE}"
WAR_FILE="${WAR_FILE}"
HOST="$HOST"
PORT="$PORT"
ENV="$ENV"
REMOTE_WAR_DIR="$REMOTE_WAR_DIR"
REMOTE_WAR_FILE="$REMOTE_WAR_FILE"
PROCESS_MODE="$process_mode"
REMOTE_TOMCAT_DIR="$REMOTE_TOMCAT_DIR"
EOF

#============CREATE ID_FILE==================
function init_ssh_id {
   if [[ ! -f "$ID_FILE" ]];then
      cat > "$ID_FILE" <<'EOF'
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
EOF
   chmod og-rwx "$ID_FILE"
   fi
}

init_ssh_id